...
def cross_validation(data, level):
    ...
    for train, val in skf.split(train_images, train_labels):
        model = setup_model()
        checkpoint = tf.keras.callbacks.ModelCheckpoint(os.path.join('models', get_model_name(level, fold)),
                                                        monitor='val_accuracy', verbose=1,
                                                        save_best_only=True, mode='max')
        callbacks_list = [checkpoint]
        model.fit(train_images[train], train_labels[train], epochs=20, validation_split=0.2,
                         callbacks=callbacks_list)
        model.load_weights(os.path.join('models', get_model_name(level, fold)))
