...
def test_process():
    trained_model = tf.keras.models.load_model('models/IntronExonClassifier.h5')
    exon_intron_rates = []
    with open("../GroupProject/dataGenerator/1_exon_5000_length.txt") as file:
        for line in file:
            if line[0] == ">":
                ratio = re.findall(r'\d[.]\d+', line)
                exon_intron_rates.append(float(ratio[0]))

    folder_dir = "../GroupProject/data/mixed/inferno/level5/mixed"
